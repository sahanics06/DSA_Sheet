...
// Solution
// from preorder we find the root and its index from inorder which gives us the left subtree starting point and ending point also we 
// get the right subtree's starting and ending point
...
public:
    TreeNode* build_tree(vector<int>& preorder, int preStart, int preEnd, vector<int>& inorder, int inStart, int inEnd, map<int,int>&mp)
    {
        ...
        int inRoot = mp[root->val]; // index of root from map
    ...
    TreeNode* buildTree(vector<int>& preorder, vector<int>& inorder) {
        ...
        TreeNode* root = build_tree(preorder, 0, preorder.size()-1,
